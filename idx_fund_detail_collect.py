...
def parse_fund_detail(content):
    ...
    '''
    fund_scale, manager_url, start_time, company = re.findall('基金规模</a>：(.*?)</td>.*基金经理：<a href="(.*?)">.*>成 立 日</span>：(.*?)<.*>管 理 人</span>：<a href=".*?">(.*?)</a>', content)[0]
    follow_error = get_follow_error(content)
    return [fund_scale, manager_url, start_time, follow_error, company]
...
def parse_manager_info_table(table):
    ...
    for row in table.find_all('tr'): 
        if count < 1:
            th = [i.text.strip() for i in row.find_all('th')]
            count+=1
            continue
        tds.append([i.text.strip() for i in row.find_all('td')]) 
...
def get_idx_fund_detail(idx_data):
    ...
    for code in idx_data['基金代码']:
        ...
        time.sleep(0.01)
    fund_detail_df = pd.DataFrame(fund_detail, columns=['基金代码', '基金规模', '基金经理url', '成立时间', '跟踪误差率', '管理人'])
    idx_fund_detail = pd.merge(idx_data, fund_detail_df, on='基金代码', how='left')
    return idx_fund_detail

if __name__ == '__main__':
    total_data = load_data(update=True)
